...
$prompt = "请联网搜索过去24小时内(今天是$today)在 GitHub Hacker News、Reddit 等技术社区被讨论、报道或热度上升的、与AI相关的开源插件、工具、MCP服务器、Claude/Agent技能。

【稳定性与质量门槛（硬性，违反任一条直接丢弃，不要进入候选）】
1. **GitHub stars 必须 ≥ 100**：低于 100 星的仓库一律不收——未经过足够数量用户实际使用验证的低星新仓，容易是半成品、玩具、fork 练手、SEO 农场，不稳定也不可靠。
2. **优先「近期被关注」而不是「近期才创建」**：项目本身要有一段历史（最好几个月以上、有多次 commit/发布记录、issue 区有真实用户反馈），只是过去24小时内被报道、登上热榜、新版本发布或被业内大 V 推荐。不要推荐「今天刚 push 第一个 commit」的全新仓库。
3. **活跃维护**：最近 60 天内有过 commit / release / issue 回复，说明作者还在维护，没有烂尾。
4. **可实际下载使用**：必须有公开代码仓库、有安装或使用方式，不是闭源商业产品、不是只有营销文案没有实物的内容。
5. **真实可信**：仓库描述与 README 内容要一致，不要 fork 自已删除/已 archive 的项目，不要内容农场。

【数量与格式】
- 以 GitHub 为主，可少量补充其他平台的开源项目
- 挑选 12 个最有价值的（系统会从中剔除重复项后精选 8 条展示，所以宁多勿少）
- 用中文输出，每个条目单独一行，格式严格为：名称 || 介绍 || 链接 || 使用方法（四个字段用两个竖线分隔，不要序号，不要 markdown 语法，不要其他任何文字）
- 介绍要求：面向普通用户，用一句通俗的话讲清楚核心功能、解决什么问题；不要罗列上线时间、星数、融资、版本号等次要信息
- 使用方法要求：用中文分号分隔成几个要点，每个要点用「标签：内容」的格式写，标签固定用：安装、部署、使用技巧、常见指令（可只写有内容的几项），尤其要说明该工具在 Claude/Claude Code 里如何安装配置使用（若是 Claude 技能或 MCP，给出安装命令和典型用法）
- 最后单独一行输出：参考来源域名：xxx"
